...
public class PermissableRelationFrame {
    ...
    private ItemStack buildAsset(String loc, String relation) {
        ...
        if (meta != null) {
            meta.setDisplayName(FactionsPlugin.getInstance().color(FactionsPlugin.getInstance().getFileManager().getFperms().getConfig().getString("fperm-gui.relation.Placeholder-Item.Name").replace("{relation}", nameCapitalized)));
