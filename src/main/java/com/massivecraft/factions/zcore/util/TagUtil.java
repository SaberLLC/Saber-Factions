...
public class TagUtil {
	...
	public static String parsePlaceholders(Player player, String line) {
		if (SaberFactions.plugin.isClipPlaceholderAPIHooked() && player.isOnline()) {
			line = PlaceholderAPI.setPlaceholders(player, line);
		}

		if (SaberFactions.plugin.isMVdWPlaceholderAPIHooked() && player.isOnline()) {
	...
	protected static List<FancyMessage> getFancy(Faction target, FPlayer fme, TagReplacer type, String prefix) {
		...
		boolean minimal = SaberFactions.plugin.getConfig().getBoolean("minimal-show", false);

		switch (type) {
			case ALLIES_LIST:
				FancyMessage currentAllies = SaberFactions.plugin.txt.parseFancy(prefix);
			...
			case ENEMIES_LIST:
				FancyMessage currentEnemies = SaberFactions.plugin.txt.parseFancy(prefix);
			...
			case TRUCES_LIST:
				FancyMessage currentTruces = SaberFactions.plugin.txt.parseFancy(prefix);
			...
			case ONLINE_LIST:
				FancyMessage currentOnline = SaberFactions.plugin.txt.parseFancy(prefix);
			...
			case OFFLINE_LIST:
				FancyMessage currentOffline = SaberFactions.plugin.txt.parseFancy(prefix);
			...
			case ALTS:
				FancyMessage alts = SaberFactions.plugin.txt.parseFancy(prefix);
	...
	private static List<String> tipFaction(Faction faction) {
		...
		for (String line : SaberFactions.plugin.getConfig().getStringList("tooltips.list")) {
	...
	private static List<String> tipPlayer(FPlayer fplayer) {
		List<String> lines = new ArrayList<>();
		for (String line : SaberFactions.plugin.getConfig().getStringList("tooltips.show")) {
