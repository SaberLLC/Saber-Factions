...
public class CmdStatus extends FCommand {
    ...
    public CmdStatus() {
        super();
        this.getAliases().addAll(Aliases.status);

        this.setRequirements(new CommandRequirements.Builder(Permission.STATUS)
                .playerOnly()
                .memberOnly()
                .build());
    }
    ...
    public void perform(CommandContext context) {
        ...
        for (FPlayer fp : context.faction.getFPlayers()) {
            ...
            ret.add(String.format(TL.COMMAND_STATUS_FORMAT.toString(), ChatColor.GOLD + fp.getRolePrefix() + fp.getName() + ChatColor.RESET, power, last).trim());
        }
